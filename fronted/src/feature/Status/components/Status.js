import React from 'react'
import Sampleavatar from '../../../assets/icons/Sampleavatar.png'
import RightArrow from '../../../assets/icons/angle-double-right.png'
import LeftArrow from '../../../assets/icons/angle-double-left.png'
import StatusPic from '../../../assets/icons/nushrat.jpg'
import SendMsg from '../../../assets/icons/sendMsg.png'
import StatusModal from './StatusModal'

const Status = () => {
  return (
    <>
    <div className='flex sm:mx-8 z-0  justify-between items-center overflow-hidden overflow-x-scroll sm:gap-x-4 gap-x-6 pt-4 pb-2 sm:px-0 px-4'>
        <div className='sm:w-20 sm:h-24 w-16 h-20 flex-col items-center justify-center'>
            <img src={Sampleavatar} className=' object-contain ring-2  p-1 ring-pink-700 rounded-full' alt="" />
            <h1 className='text-center font-semibold'>Nishant</h1>
        </div>
        <div className='sm:w-20 sm:h-24 w-16 h-20 flex-col items-center justify-center'>
            <img src={Sampleavatar} className=' object-contain ring-2  p-1 ring-pink-700 rounded-full' alt="" />
            <h1 className='text-center font-semibold'>Nishant</h1>
        </div>
        <div className='sm:w-20 sm:h-24 w-16 h-20 flex-col items-center justify-center'>
            <img src={Sampleavatar} className=' object-contain ring-2  p-1 ring-pink-700 rounded-full' alt="" />
            <h1 className='text-center font-semibold'>Nishant</h1>
        </div>
        <div className='sm:w-20 sm:h-24 w-16 h-20 flex-col items-center justify-center'>
            <img src={Sampleavatar} className=' object-contain ring-2  p-1 ring-pink-700 rounded-full' alt="" />
            <h1 className='text-center font-semibold'>Nishant</h1>
        </div>
        <div className='sm:w-20 sm:h-24 w-16 h-20 flex-col items-center justify-center'>
            <img src={Sampleavatar} className=' object-contain ring-2  p-1 ring-pink-700 rounded-full' alt="" />
            <h1 className='text-center font-semibold'>Nishant</h1>
        </div>
        <div className='sm:w-20 sm:h-24 w-16 h-20 flex-col items-center justify-center'>
            <img src={Sampleavatar} className=' object-contain ring-2  p-1 ring-pink-700 rounded-full' alt="" />
            <h1 className='text-center font-semibold'>Nishant</h1>
        </div>
        <div className='sm:w-20 sm:h-24 w-16 h-20 flex-col items-center justify-center'>
            <img src={Sampleavatar} className=' object-contain ring-2  p-1 ring-pink-700 rounded-full' alt="" />
            <h1 className='text-center font-semibold'>Nishant</h1>
        </div>
        <div className='sm:w-20 sm:h-24 w-16 h-20 flex-col items-center justify-center'>
            <img src={Sampleavatar} className=' object-contain ring-2  p-1 ring-pink-700 rounded-full' alt="" />
            <h1 className='text-center font-semibold'>Nishant</h1>
        </div>
        <div className='sm:w-20 sm:h-24 w-16 h-20 flex-col items-center justify-center'>
            <img src={Sampleavatar} className=' object-contain ring-2  p-1 ring-pink-700 rounded-full' alt="" />
            <h1 className='text-center font-semibold'>Nishant</h1>
        </div>
        <div className='sm:w-20 sm:h-24 w-16 h-20 flex-col items-center justify-center'>
            <img src={Sampleavatar} className=' object-contain ring-2  p-1 ring-pink-700 rounded-full' alt="" />
            <h1 className='text-center font-semibold'>Nishant</h1>
        </div>
        <div className='sm:w-20 sm:h-24 w-16 h-20 flex-col items-center justify-center'>
            <img src={Sampleavatar} className=' object-contain ring-2  p-1 ring-pink-700 rounded-full' alt="" />
            <h1 className='text-center font-semibold'>Nishant</h1>
        </div>
    </div>
    <StatusModal/>  
    </>
  )
}

export default Status