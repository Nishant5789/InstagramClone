    ...
    <div className='overflow-scroll mx-2 row-span-4'>
      {
        CurrMsgData.map(({SenderUserId, ContentMessage, ContentType, ChatId}) => {
          return (
            <div className={`flex  my-4 ${SenderUserId===getLoggeduserId() ? "justify-end":"justify-start"}`}>
              {/* // rounded-r-lg rounded-bl-lg  || rounded-l-lg rounded-br-lg */}
              <div className={`${SenderUserId===getLoggeduserId() ? "rounded-l-lg rounded-br-lg":"rounded-r-lg rounded-bl-lg"} bg-purple-600 max-w-[20rem] text-white shadow-md shadow-slate-500 hover:bg-purple-500  px-3 py-2`}>
                <p>{ContentMessage}</p>
              </div>
            </div>
          )
